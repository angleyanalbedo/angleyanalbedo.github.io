...
首先我们需要从`hadoop`下载数据集到本地电脑可以使用网页下载也可以可以使用命令行进行下载
...
美妆数据处理的代码都在`ana/makeupana.ipynb`中

![image-20240501092317955](美妆分析系统整体流程/image-20240501092317955.png)

所有以下的代码都是按顺序写的

- 首先是数据下载

  ![image-20240501092836466](美妆分析系统整体流程/image-20240501092836466.png)
...
![image-20240501092921577](美妆分析系统整体流程/image-20240501092921577.png)
...
2. 在数据分析和数据可视化之后我们会得到可视化的结果结果就是一张一张的图片，这些图片你在数据makeupana.ipyb 运行的过程中你就会看到就是那些得到的结果。

##### 给商品名称中文分词

![image-20240501093316566](美妆分析系统整体流程/image-20240501093316566.png)
...
![image-20240501093419481](美妆分析系统整体流程/image-20240501093419481.png)
...
![image-20240501093505928](美妆分析系统整体流程/image-20240501093505928.png)
...
![image-20240501093536858](美妆分析系统整体流程/image-20240501093536858.png)

添加一列是不是男性专用

##### 计算销量之类的

![image-20240501093559927](美妆分析系统整体流程/image-20240501093559927.png)
...
![image-20240501093651356](美妆分析系统整体流程/image-20240501093651356.png)
...
![](美妆分析系统整体流程/image-20240420144954733.png)

##### 品牌销售量

![image-20240501093717799](美妆分析系统整体流程/image-20240501093717799.png)
...
![image-20240420151747132](美妆分析系统整体流程/image-20240420151747132.png)
...
2. 第二行代码:
   ...
   ```
   这行代码使用Seaborn库的`barplot`函数创建了一个条形图。`x`参数设置为"店名"，`y`参数设置为"sale_count"，`hue`参数设置为"main_type"，表示将根据"main_type"的取值进行分组。`data`参数指定要使用的数据集，这里使用了名为`data`的数据集。`saturation`参数设置饱和度为0.75，`ci`参数设置为0，表示不显示误差线。
...
5. 第五行代码:
   ...
   ```
   这行代码在图形中添加了一段文本，位于坐标(0, 78000)的位置。文本内容是"注：此处也可使用堆叠图，对比效果更直观"。`verticalalignment`参数设置垂直对齐方式为"top"，`horizontalalignment`参数设置水平对齐方式为"left"，`color`参数设置文本颜色为灰色，`fontsize`参数设置文本字体大小为10。
...
综合起来，这段代码的目的是使用条形图对数据进行可视化，横轴表示店名，纵轴表示销售数量，颜色的不同代表不同的主要类型。图形的标题是"各品牌各总类的总销量"，纵轴标签为"销量"。此外，还在图形中添加了一段注释文本，提醒读者也可使用堆叠图来观察数据的对比效果。![image-20240501094658726](美妆分析系统整体流程/image-20240501094658726.png)

![image-20240501094711082](美妆分析系统整体流程/image-20240501094711082.png)

##### 销量

![image-20240501093749303](美妆分析系统整体流程/image-20240501093749303.png)

使用`matplot`创建柱状图

![image-20240420152138014](美妆分析系统整体流程/image-20240420152138014.png)

##### 销售额

各品牌各总类总销售额

![image-20240501095128187](美妆分析系统整体流程/image-20240501095128187.png)

各品牌子类总销售额

![image-20240501094941786](美妆分析系统整体流程/image-20240501094941786.png)

各品牌各子类总销售额

##### 各销售物品的平均评论数

![image-20240501093858249](美妆分析系统整体流程/image-20240501093858249.png)
...
![image-20240501093918660](美妆分析系统整体流程/image-20240501093918660.png)
...
![image-20240420154606255](美妆分析系统整体流程/image-20240420154606255.png)

##### 价格箱型图

![image-20240501093938689](美妆分析系统整体流程/image-20240501093938689.png)
...
![image-20240420154549889](美妆分析系统整体流程/image-20240420154549889.png)

##### 个品牌的平均价格

![image-20240501093956814](美妆分析系统整体流程/image-20240501093956814.png)
...
![image-20240501094030190](美妆分析系统整体流程/image-20240501094030190.png)
...
![image-20240420155027534](美妆分析系统整体流程/image-20240420155027534.png)

##### 销量价格之间关系

![image-20240501094100260](美妆分析系统整体流程/image-20240501094100260.png)
...
![image-20240420160038024](美妆分析系统整体流程/image-20240420160038024.png)

##### 是否为男士专用

![image-20240501094211266](美妆分析系统整体流程/image-20240501094211266.png)
...
1. 第一行代码:
   ...
   ```
   这行代码的目的是从名为`data`的数据集中筛选出"是否男士专用"这一列的取值为"是"的数据，并将结果存储在名为`gender_data`的新数据集中。

2. 第二行代码:
   ```
   gender_data_1=gender_data[(gender_data.main_type =='护肤品')| (gender_data.main_type=='化妆品')]
   ```
   这行代码的目的是从上一步筛选得到的`gender_data`数据集中，进一步筛选出"main_type"这一列的取值为"护肤品"或"化妆品"的数据，并将结果存储在名为`gender_data_1`的新数据集中。
...
4. 第四行代码:
   ...
   ```
   这行代码使用Seaborn库的`barplot`函数创建了一个条形图。`x`参数设置为"店名"，`y`参数设置为"sale_count"，`hue`参数设置为"main_type"，表示将根据"main_type"的取值进行分组。`data`参数指定要使用的数据集，这里使用了之前筛选得到的`gender_data_1`数据集。`saturation`参数设置饱和度为0.75，`ci`参数设置为0，表示不显示误差线。
...
![image-20240501094405071](美妆分析系统整体流程/image-20240501094405071.png)

##### 男生的销量与销售额

![image-20240501094423561](美妆分析系统整体流程/image-20240501094423561.png)
...
![image-20240420160241804](美妆分析系统整体流程/image-20240420160241804.png)

##### 销量随时间的变化的变化

![image-20240501094444306](美妆分析系统整体流程/image-20240501094444306.png)
...
![image-20240420160602719](美妆分析系统整体流程/image-20240420160602719.png)
...
![image-20240430184813282](美妆分析系统整体流程/image-20240430184813282.png)

第一个分析在`makeup-ui/src/views/system/ana1/index.vue`
...
![image-20240430185117173](美妆分析系统整体流程/image-20240430185117173.png)
